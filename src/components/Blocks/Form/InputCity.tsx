...
export const InputCityBlock = ({ block }: InputCityProps) => {
  ...
  return (
    <div style={{ padding: '10px', width: '100%', height: '100%', ...style }}>
      <div style={{ marginBottom: '8px' }}>
        ...
        <input
          type="text"
          id={`input-city-${block.id}`}
          placeholder={content.placeholder || 'Şehir giriniz'}
        ...
        {isInvalid && content.showError && (
          <div style={{ color: '#e53e3e', fontSize: '12px', marginTop: '4px' }}>
            {content.errorMessage || 'Lütfen şehir giriniz'}
          </div>
        )}
      </div>
    </div>
